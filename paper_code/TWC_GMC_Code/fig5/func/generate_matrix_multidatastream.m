function [A,b,m,n,find_submat] = generate_matrix_multidatastream(connected_links, trans_cache_set, receiver_cache_set, data_stream)
...
%                         fprintf('zeros_num:%d\n',zeros_num);
    ...
    function [row,col]=find_submat_func(k,j,i)
        col = (sum(trans_cache_set_size(1:j-1))+find(cache_set{j}==i)-1)*data_stream+[1:data_stream];
        row = (k-1)*data_stream+[1:data_stream];
    end
find_submat = @find_submat_func;
    one_ind = 0;
